...
let make = () => {
  ...
  switch (response) {
    ...
    | Data(d) => /* Render UI with data. d represents the data returned by your GraphQL subscription.  */
...
| Argument  | Type            | Description                                                                                                                                                                                                                                                                 |
| --------- | --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `request` | `Types.request` | Your `graphql_ppx` request representing your mutation. Generated by calling `.make()` on the `graphql_ppx` module. If you're not using `graphql_ppx`, pass a `Js.t` of the following shape: `{. "query": string, "variables": Js.Json.t, "parse": Js.Json.t => 'response }` |
...
| Argument  | Type            | Description                                                                                                                                                                                                                                                                     |
| --------- | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `request` | `Types.request` | Your `graphql_ppx` request representing your subscription. Generated by calling `.make()` on the `graphql_ppx` module. If you're not using `graphql_ppx`, pass a `Js.t` of the following shape: `{. "query": string, "variables": Js.Json.t, "parse": Js.Json.t => 'response }` |
...
The `Query` allows you to query your GraphQL API and render UI with the returned data.
...
| `executeQuery` | `option(Js.Json.t) => Js.Promise.t(Types.operationResult)` | A callback to imperatively re-execute the query operation. Accepts a partial operation context to modify execution of the query.        |
| `response`     | `Types.response('response)`                                | A variant containing constructors for `Data`, `Error`, `Fetching` and `NotFound`. Useful for pattern matching to render conditional UI. |
...
let forwardSubscription = operation => subClient##request(operation);
...
let query = SubscribeMessages.make();
...
| `fetchOptions` | `option(fetchOptions)=?`              | A variant type representing optional fetch options to be used by your client. You can pass your `fetchOptions` as a plain `Fetch.requestInit` by wrapping it in `Client.FetchOpts`, or instantiate it dynamically in a function wrapped by `Client.FetchFn`. |
