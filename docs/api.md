...
`reason-urql` has a nice set of custom hooks to use in your ReasonReact components, including `useQuery`, `useMutation`, and `useSubscription`. These are fully type safe and will correctly infer the type of your GraphQL response if using `graphql_ppx`.
...
`useQuery` returns a tuple containing the result of executing your GraphQL query using a record, `hookResponse`, and a function for re-executing the query imperatively, `executeQuery`.
...
let make = () => {
  ...
  switch (response) {
    ...
    | Data(d) => /* Render UI with data. d will  */
