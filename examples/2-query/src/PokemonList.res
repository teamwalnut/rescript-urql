...
let filterPokemon = (input: string, pokemon: array<string>) =>
  ...
  |> Array.to_list
  |> List.filter(pokemon =>
    Js.String.includes(String.lowercase_ascii(input), String.lowercase_ascii(pokemon))
  )
...
let make = (~pokemon) => {
  let (state, dispatch) = React.useReducer((state, action) =>
    switch action {
    | ChangeInput(search) => {...state, search: search}
  ...
  <div className="sidebar-container">
    <section className="sidebar">
      <input
        ...
        value=state.search
        onChange={event => dispatch(ChangeInput((event->ReactEvent.Form.target)["value"]))}
      ...
      <ul className="sidebar__list">
        ...
        |> List.map(pokemon =>
          <li key=pokemon className="sidebar__list-item">
            <button
              className="sidebar__button" onClick={_event => dispatch(SelectPokemon(pokemon))}>
