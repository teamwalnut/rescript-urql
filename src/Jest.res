...
module Runner = (A: Asserter) => {
  ...
  @val
  external _testPromise: (string, (. unit) => Js.Promise.t<'a>, Js.Undefined.t<int>) => unit =
    "test"
  ...
  let testPromise = (name, ~timeout=?, callback) =>
    _testPromise(
      name,
      (. ()) => callback()->Js.Promise.then_(a => a->A.affirm->Js.Promise.resolve, _),
      Js.Undefined.fromOption(timeout),
    )

  let testAll = (name, inputs, callback) =>
    inputs->Belt.List.forEach(((inputStr, other)) => {
      let name = `${name} - ${inputStr}`
      _test(name, (. ()) => {
        affirm(callback(inputStr, other))
        Js.undefined
      })
    })
  ...
  @val external _beforeAll: (. (. unit) => unit) => unit = "beforeAll"
  let beforeAll = fn => _beforeAll((. ()) => fn())
  ...
  @val
  external beforeAllPromise: ((. unit) => Js.Promise.t<'a>, Js.Undefined.t<int>) => unit =
    "beforeAll"
  ...
  let beforeEach = fn => _beforeEach(.(. ()) => fn())
  ...
  @val
  external beforeEachPromise: ((. unit) => Js.Promise.t<'a>, Js.Undefined.t<int>) => unit =
    "beforeEach"
  let beforeEachPromise = (~timeout=?, callback) =>
    beforeEachPromise((. ()) => Js.Promise.resolve(callback()), Js.Undefined.fromOption(timeout))

  @val external _afterAll: (. (. unit) => unit) => unit = "afterAll"
  let afterAll = fn => _afterAll((. ()) => fn())
  ...
  @val external _afterEach: (. (. unit) => unit) => unit = "afterEach"
  let afterEach = fn => _afterEach((. ()) => fn())
  ...
  module Only = {
    ...
    let testPromise = (name, ~timeout=?, callback) =>
      _testPromise(
        name,
        (. ()) => callback()->Js.Promise.then_(a => a->affirm->Js.Promise.resolve, _),
        Js.Undefined.fromOption(timeout),
      )
  ...
  module Skip = {
    ...
    let testAll = (name, inputs, callback) =>
      inputs->Belt.List.forEach(((inputStr, other)) => {
        let name = `${name} - ${inputStr}`
        test(name, () => callback(inputStr, other))
      })
    @val
...
@val external usingFakeTimers: unit => unit = "jest.useFakeTimers"
@val
external usingFakeTimersImplementation: fakeTimerImplementation => unit = "jest.useFakeTimers"
let usingFakeTimers = (~implementation: option<fakeTimerImplementation>=?, ()) => {
  switch implementation {
  | None => usingFakeTimers()
  | Some(implString) => usingFakeTimersImplementation(implString)
  }
}
@val external usingRealTimers: unit => unit = "jest.useRealTimers"
